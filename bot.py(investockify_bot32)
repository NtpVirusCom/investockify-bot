#!/usr/bin/env python3
"""
🤖 Investockify Bot — Railway Cost-Optimized Edition (v2.5.0)
==========================================================v.32==
...
THAI_NAME = {
    "PTT": "ปตท.", "AOT": "ท่าอากาศยานไทย", "ADVANC": "AIS", "CPALL": "CPALL",
    "KBANK": "กสิกรไทย", "SCB": "SCBX", "BDMS": "BDMS", "BBL": "ธนาคารกรุงเทพ",
    "CPN": "เซ็นทรัลพัฒนา", "CRC": "เซ็นทรัลรีเทล", "DELTA": "เดลต้าอีเลคโทรนิคส์",
    "GPSC": "โกลบอลเพาเวอร์", "GULF": "กัลฟ์", "HMPRO": "โฮมโปร", "IVL": "อินดอร์มา",
    "KTB": "กรุงไทย", "KTC": "บัตรกรุงไทย", "MINT": "ไมเนอร์อินเตอร์เนชั่นแนล",
    "OR": "OR", "PTTEP": "ปตท.สำรวจ", "PTTGC": "PTTGC", "SCC": "ปูนซิเมนต์ไทย",
    "SCGP": "เอสซีจีแพคเกจจิ้ง", "THAI": "การบินไทย", "TLI": "ไทยประกันชีวิต",
    "TRUE": "ทรู", "TTB": "ทหารไทย", "AP": "เอพี", "AMATA": "อมตะนคร",
    "EGCO": "อีจีซีโอ", "LH": "แลนด์แอนด์เฮ้าส์", "ROJNA": "รุ่งนภา", "SPALI": "ศุภาลัย",
    "TTW": "ทีทีดับบลิว", "BH": "โรงพยาบาลบำรุงราษฎร์", "CPF": "ซีพีเอฟ",
    "DIF": "DIF", "BTS": "บีทีเอส", "TOP": "ไทยออยล์", "IRPC": "ไออาร์พีซี",
    "COM7": "คอมเซเว่น", "CPAXT": "ซีพีแอ็กซ์ตร้า", "RBF": "อาร์แอนด์บีฟู้ด",
    "NER": "นอร์ทอีสรับเบอร์", "JMT": "เจเอ็มทีเน็ทเวอร์ค", "JMART": "เจเอ็มที",
    "SINGER": "สิงห์", "MTC": "เงินติดล้อ", "SAWAD": "ศรีสวัสดิ์",
    "KCE": "เคซีอี", "HANA": "ฮาน่า", "SMT": "เอสเอ็มที",
    "BCH": "โรงพยาบาลเกษมราษฎร์", "RML": "รุ่งเรืองผลมอเตอร์",
    "PR9": "พรีเมียร์", "SYNEX": "ซินเน็ค", "ASIAN": "เอเชียน",
    "GLOBAL": "สยามเวลเนส", "JAS": "จัสมิน", "INET": "ไอเน็ต",
    "SISB": "เอสไอเอสบี", "PR9": "พรีเมียร์การประชุม",
}
...
def fetch_newsapi(query: str, max_items: int = 5, language: str = "en") -> list:
    """Fetch from NewsAPI (good for US stocks and commodities)"""
    if not NEWSAPI_KEY:
        return []
    try:
        params = {
        ...
        for article in articles:
            pub_time = _parse_timestamp(article.get("publishedAt"))
            results.append({
...
def fetch_finnhub(symbol: str, max_items: int = 5) -> list:
    """Fetch from Finnhub (good for US stocks)"""
    if not FINNHUB_KEY:
        return []
    try:
...
def fetch_coingecko_news(max_items: int = 5) -> list:
    """Fetch crypto news from CoinGecko (free, no key)"""
    try:
...
def fetch_cryptocompare_news(crypto_symbol: str, max_items: int = 5) -> list:
    """Fetch from CryptoCompare (free tier available)"""
    try:
        # Map symbol to CryptoCompare format
        base = crypto_symbol.replace("-USD", "")
        ...
        for item in data.get("Data", [])[:max_items]:
            results.append({
                ...
                "link": item.get("url", ""),
                "published_ms": item.get("published_on", 0) * 1000  # Unix timestamp in seconds
            })
        return results
    except Exception as e:
...
def fetch_kaohoon_news(stock_code: str, max_items: int = 5) -> list:
    """
    Fetch from Kaohoon.com (Thai stock news)
    ...
    try:
        ...
        # Simple regex extraction (Kaohoon structure may change)
        import re
        ...
        for link, title in matches[:max_items]:
            ...
            if clean_title:
                results.append({
                    "title": clean_title,
                    "publisher": "Kaohoon",
                    "link": link if link.startswith("http") else f"https://www.kaohoon.com{link}",
                    "published_ms": 0  # Kaohoon doesn't expose time easily
                })
        return results
    except Exception as e:
        logger.warning("Kaohoon fetch failed: %s", e)
        return []


def fetch_set_news(stock_code: str, max_items: int = 5) -> list:
    """Fetch from SET.or.th (official Thai stock exchange)"""
    try:
        # SET news RSS or search
        url = f"https://www.set.or.th/th/market/news-and-events/news.html"
        # This would require proper scraping with BeautifulSoup
        # Placeholder for implementation
        return []
    except Exception as e:
        logger.warning("SET news fetch failed: %s", e)
        return []
...
def fetch_stock_news(symbol: str, max_items: int = 5) -> list:
    ...
    optimized per asset class
    """
    market = detect_market(symbol)
    all_news = []
    
    if market == "US":
        # Chain: Finnhub → Yahoo Finance → NewsAPI
        if FINNHUB_KEY:
            all_news = fetch_finnhub(symbol, max_items)
        if not all_news:
            all_news = fetch_yahoo_news(symbol, max_items)
        if not all_news and NEWSAPI_KEY:
            # Clean symbol for search (remove .US, etc.)
            search_term = symbol.replace(".US", "").replace("-", " ")
            all_news = fetch_newsapi(search_term, max_items)
            
    elif market == "TH":
        # Chain: Kaohoon → Yahoo Finance → Google News (fallback)
        ...
        if not all_news:
            all_news = fetch_yahoo_news(symbol, max_items)
        if not all_news:
            # Use Thai name for Google News
            all_news = fetch_google_news(thai_name, max_items)
            
    elif market == "CRYPTO":
        # Chain: CoinGecko → CryptoCompare → CoinDesk RSS → Yahoo
        ...
        # Chain: Yahoo Finance → NewsAPI → Google News
        keyword = COMMODITY_KEYWORDS.get(symbol, symbol.replace("=F", ""))
        
        all_news = fetch_yahoo_news(symbol, max_items)
        if not all_news and NEWSAPI_KEY:
            all_news = fetch_newsapi(keyword, max_items)
        if not all_news:
...
def fetch_google_news(query,max_items=5):
    ...
    url = GOOGLE_NEWS.format(query=query)
    ...
    for entry in feed.entries[:max_items]:
        ...
        results.append({
            ...
            "published_ms":int(datetime(*entry.published_parsed[:6]).timestamp()*1000)
...
def fetch_stock_news(symbol,max_items=5):

    market=detect_market(symbol)

    if market=="US":

        return fetch_yahoo_news(symbol,max_items)

    elif market=="TH":

        code=symbol.replace(".BK","")

        keyword=THAI_NAME.get(code,code)

        news=fetch_google_news(keyword,max_items)
        ...
        keyword=symbol.replace("=F","")

        return fetch_yahoo_news(keyword,max_items)

    elif market=="CRYPTO":

        keyword=symbol.replace("-USD","")

        return fetch_google_news(keyword,max_items)
