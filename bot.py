#!/usr/bin/env python3
"""
🤖 Investockify Bot — Railway Cost-Optimized Edition (v2.3.0)
==========================================================v.28==
...
THAI_STOCKS: Set[str] = {
    ...
    "AP", "AMATA", "EGCO", "LH", "ROJNA", "SCB", "SCC", "SPALI",
    "TTW"
}
...
# Enhanced Edition — Multi-Source per Asset Class
# ไม่ใช้ NewsAPI — ใช้แหล่งฟรีและเฉพาะทางแทน
# =========================================================
...
# =========================================================
# ENHANCED NEWS SOURCES CONFIG
# =========================================================

# Thai Stock Name Mapping (expanded to all 40+ stocks)
THAI_NAME = {
    "PTT": "ปตท.", "AOT": "ท่าอากาศยานไทย", "ADVANC": "AIS", "CPALL": "CP ALL",
    "KBANK": "กสิกรไทย", "SCB": "SCBX", "BDMS": "BDMS", "BBL": "ธนาคารกรุงเทพ",
    "CPN": "เซ็นทรัลพัฒนา", "CRC": "เซ็นทรัล รีเทล", "DELTA": "เดลต้า อีเลคโทรนิคส์",
    "GPSC": "โกลบอล เพาเวอร์", "GULF": "กัลฟ์", "HMPRO": "โฮม โปร", "IVL": "อินดอร์มา",
    "KTB": "กรุงไทย", "KTC": "บัตรกรุงไทย", "MINT": "ไมเนอร์ อินเตอร์เนชั่นแนล",
    "OR": "OR", "PTTEP": "ปตท.สำรวจ", "PTTGC": "PTTGC", "SCC": "ปูนซิเมนต์ไทย",
    "SCGP": "เอสซีจี แพคเกจจิ้ง", "THAI": "การบินไทย", "TLI": "ไทยประกันชีวิต",
    "TRUE": "ทรู", "TTB": "ทหารไทย", "AP": "เอพี", "AMATA": "อมตะ นคร",
    "EGCO": "อีจีซีโอ", "LH": "แลนด์ แอนด์ เฮ้าส์", "ROJNA": "รุ่งนภา", "SPALI": "ศุภาลัย",
    "TTW": "ทีทีดับบลิว", "BH": "โรงพยาบาลบำรุงราษฎร์", "CPF": "ซีพีเอฟ",
    "DIF": "DIF", "BTS": "บีทีเอส"
}

# Commodity keyword mapping for better search
COMMODITY_KEYWORDS = {
    "GC=F": "gold price",
    "SI=F": "silver price",
    "HG=F": "copper price",
...
# Finnhub config (optional, free tier: 60 req/min)
FINNHUB_KEY  = os.environ.get("FINNHUB_KEY")
FINNHUB_URL = "https://finnhub.io/api/v1/company-news"

# CoinGecko config (free, no key needed)
...
# =========================================================
# SENTIMENT PATTERNS (keep existing)
# =========================================================
BULLISH_PATTERNS = [
    r'(surge|surges|surged|rally|rallies|rallied|soar|soars|soared|jump|jumps|jumped|climb|climbs|climbed|skyrocket|skyrockets|skyrocketed)',
    r'(beat|beats|beating).*(earnings|revenue|expectation|forecast|consensus)',
    r'(upgrade|upgrades|upgraded)',
    r'(strong|stronger|strongest).*(growth|demand|sales|performance|quarter|guidance)',
    r'(buy|overweight|outperform).*(rating|target|price target|recommendation)',
    r'(exceed|exceeds|exceeded|outperform|outperforms|outperformed)',
    r'(positive|optimistic|promising|recovery|expansion|breakthrough|record high)',
    r'(raise|raises|raised|lifting|lifts).*(guidance|outlook|target|forecast)',
    r'(approval|approved|clearance|cleared|launch|launches|launched)',
]

BEARISH_PATTERNS = [
    r'(plunge|plunges|plunged|crash|crashes|crashed|tank|tanks|tanked|dive|dives|dived|slump|slumps|slumped|collapse|collapses|collapsed)',
    r'(miss|misses|missed).*(earnings|revenue|target|forecast|consensus|expectation)',
    r'(downgrade|downgrades|downgraded)',
    r'(weak|weaker|weakest).*(demand|sales|performance|quarter|guidance|outlook)',
    r'(sell|underweight|underperform).*(rating|target|price target|recommendation)',
    r'(layoff|layoffs|laying off|lay off|job cuts|workforce reduction)',
    r'(debt|debts|loss|losses|deficit|bankruptcy|bankrupt|default|defaults)',
    r'(cut|cuts|cutting|slash|slashes|slashed|lower|lowers|lowered).*(guidance|outlook|target|forecast|dividend)',
    r'(negative|pessimistic|disappointing|warning|downturn|correction|recession|bearish)',
    r'(investigation|investigating|sec probe|lawsuit|litigation|fine|penalty|scandal)',
]
    ...
    if use_ai_fallback and OPENAI_API_KEY and abs(bull_count - bear_count) <= 1:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[{
                    "role": "user",
                    "content": f'Analyze sentiment of this financial news headline. Reply ONLY with: BULLISH, BEARISH, or NEUTRAL."{title}"'
                }],
                max_tokens=10,
                temperature=0.0
            )
...
# =========================================================
# ENHANCED FETCH FUNCTIONS (No NewsAPI)
# =========================================================

def fetch_finnhub(symbol: str, max_items: int = 5) -> list:
    """Fetch from Finnhub (good for US stocks) — optional, needs FINNHUB_KEY"""
    if not FINNHUB_KEY:
        return []
    try:
        end = int(datetime.now().timestamp())
        start = end - 30 * 24 * 60 * 60  # 30 days back
        params = {
            "symbol": symbol,
        ...
        for item in data[:max_items]:
            pub_time = _parse_timestamp(item.get("datetime"))
            results.append({
...
def fetch_coingecko_news(max_items: int = 5) -> list:
    ...
    try:
        ...
        for item in data.get("data", [])[:max_items]:
            results.append({
                ...
                "publisher": item.get("source", "CoinGecko"),
                "link": item.get("url", ""),
                "published_ms": _parse_timestamp(item.get("published_at")) or 0
            })
        return results
    except Exception as e:
...
def fetch_cryptocompare_news(crypto_symbol: str, max_items: int = 5) -> list:
    """Fetch from CryptoCompare (free tier available, no key for basic)"""
    try:
        base = crypto_symbol.replace("-USD", "").replace("USDT", "")
        ...
        for item in data.get("Data", [])[:max_items]:
            results.append({
                "title": item.get("title", ""),
                "publisher": item.get("source", "CryptoCompare"),
                "link": item.get("url", ""),
                "published_ms": item.get("published_on", 0) * 1000
            })
        return results
    except Exception as e:
        logger.warning("CryptoCompare fetch failed for %s: %s", crypto_symbol, e)
        return []


def fetch_coindesk_rss(max_items: int = 5) -> list:
    """Fetch from CoinDesk RSS (crypto specific, free)"""
    try:
        ...
        for entry in feed.entries[:max_items]:
            ...
            if hasattr(entry, "published_parsed") and entry.published_parsed:
                pub_time = int(datetime(*entry.published_parsed[:6]).timestamp() * 1000)
            results.append({
                "title": entry.title,
                "publisher": "CoinDesk",
                "link": entry.link,
...
def fetch_cointelegraph_rss(max_items: int = 5) -> list:
    """Fetch from Cointelegraph RSS (crypto specific, free)"""
    try:
...
def fetch_kaohoon_news(stock_code: str, max_items: int = 5) -> list:
    """
    Fetch from Kaohoon.com (Thai stock news) — web scraping
    Note: Structure may change, handle gracefully
    """
    try:
        url = f"https://www.kaohoon.com/?s={quote(stock_code)}"
        headers = {
        ...
        pattern = r'<h[23][^>]*>.*?<a[^>]*href="([^"]*)"[^>]*>(.*?)</a>.*?</h[23]>'
        matches = re.findall(pattern, resp.text, re.DOTALL)

        results = []
        for link, title in matches[:max_items]:
            clean_title = re.sub(r'<[^>]+>', '', html.unescape(title)).strip()
            if clean_title:
                full_link = link if link.startswith("http") else f"https://www.kaohoon.com{link}"
                results.append({
                    "title": clean_title,
                    "publisher": "Kaohoon",
                    "link": full_link,
        ...
        logger.warning("Kaohoon fetch failed for %s: %s", stock_code, e)
        return []


def fetch_investing_rss(commodity_keyword: str, max_items: int = 5) -> list:
    """Fetch from Investing.com RSS for commodities"""
    try:
        # Investing.com has RSS feeds for commodities
        feed_url = f"https://www.investing.com/rss/news_{commodity_keyword}.rss"
        ...
        logger.warning("Investing.com RSS failed: %s", e)
        return []


def fetch_google_news(query: str, max_items: int = 5) -> list:
    """Fetch from Google News RSS (fallback, free but unstable)"""
    try:
        url = GOOGLE_NEWS.format(query=quote(query))
...
def fetch_yahoo_news(symbol: str, max_items: int = 5) -> list:
    """Fetch from Yahoo Finance (existing, good for US stocks)"""
    try:
        ticker = yf.Ticker(symbol)
...
# =========================================================
# MARKET DETECTION (Enhanced)
# =========================================================

def detect_market(symbol: str) -> str:
    """Enhanced market detection for routing news sources"""
    if symbol.endswith(".BK"):
        return "TH"
    if symbol.endswith("-USD") or symbol.endswith("USDT") or symbol in CRYPTO_PAIRS:
        return "CRYPTO"
    if symbol.endswith("=F"):
        return "COMMODITY"
    # Additional patterns
    if any(crypto in symbol for crypto in ["BTC", "ETH", "SOL", "BNB", "XRP"]):
        return "CRYPTO"
    if any(comm in symbol for comm in ["GC=", "SI=", "CL=", "NG=", "HG="]):
        return "COMMODITY"
    return "US"
...
def fetch_stock_news(symbol: str, max_items: int = 5) -> list:
    """
    Enhanced news fetcher with multi-source fallback chain
    optimized per asset class — NO NewsAPI used
    """
    market = detect_market(symbol)
    all_news = []

    if market == "US":
        # Chain: Finnhub (if key) → Yahoo Finance → Google News (fallback)
        if FINNHUB_KEY:
            all_news = fetch_finnhub(symbol, max_items)
        if not all_news:
            all_news = fetch_yahoo_news(symbol, max_items)
        if not all_news:
            search_term = symbol.replace(".US", "").replace("-", " ")
            all_news = fetch_google_news(search_term, max_items)

    elif market == "TH":
        # Chain: Kaohoon → Yahoo Finance → Google News (with Thai name)
        code = symbol.replace(".BK", "")
        thai_name = THAI_NAME.get(code, code)

        all_news = fetch_kaohoon_news(code, max_items)
        if not all_news:
            all_news = fetch_yahoo_news(symbol, max_items)
        if not all_news:
            all_news = fetch_google_news(thai_name, max_items)

    elif market == "CRYPTO":
        # Chain: CoinGecko → CryptoCompare → CoinDesk → Cointelegraph → Yahoo
        keyword = CRYPTO_KEYWORDS.get(symbol, symbol.replace("-USD", ""))

        all_news = fetch_coingecko_news(max_items)
        if not all_news:
            all_news = fetch_cryptocompare_news(symbol, max_items)
        if not all_news:
            all_news = fetch_coindesk_rss(max_items)
        if not all_news:
            all_news = fetch_cointelegraph_rss(max_items)
        if not all_news:
            all_news = fetch_yahoo_news(symbol, max_items)

    elif market == "COMMODITY":
        # Chain: Yahoo Finance → Investing.com RSS → Google News
        keyword = COMMODITY_KEYWORDS.get(symbol, symbol.replace("=F", ""))

        all_news = fetch_yahoo_news(symbol, max_items)
        if not all_news:
            all_news = fetch_investing_rss(keyword, max_items)
        if not all_news:
            all_news = fetch_google_news(keyword, max_items)

    # Deduplicate by title (case-insensitive, ignore short titles)
    seen = set()
    deduped = []
    for item in all_news:
        title_key = item["title"].lower().strip()
        if title_key and title_key not in seen and len(title_key) > 10:
            seen.add(title_key)
            deduped.append(item)

    # Sort by recency (items with 0 timestamp go to end)
    deduped.sort(key=lambda x: x.get("published_ms", 0), reverse=True)

    return deduped[:max_items]


# =========================================================
# SENTIMENT ANALYSIS (keep existing)
# =========================================================
...
def _generate_ai_thai_summary(news_items: list, symbol: str) -> str:
    ...
    with lock:
        ...
        try:
            import openai
            client = openai.OpenAI(api_key=OPENAI_API_KEY)
            headlines = "".join([f"- {item['title']}" for item in news_items[:5]])
...
# =========================================================
# FORMAT NEWS SECTION (keep existing structure, use enhanced fetcher)
# =========================================================
