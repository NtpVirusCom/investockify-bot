#!/usr/bin/env python3     ***investockify_bot12.py***
"""
🤖 Investockify Bot — Railway Production Edition (v2.0.3)
============================================================
ปรับปรุงจาก v2.0.2:
  • ลบ API Keys ที่ฝังในโค้ด (Security Hardened)
  • Input Sanitization ด้วย unicodedata + regex รัดกุม
  • Hybrid News Scoring: Regex Pattern (เร็ว) + AI Fallback (แม่นยำ)
  • DataFetcher LRU Cache + Memory Limit สำหรับ Railway Container
  • Graceful Shutdown (SIGTERM/SIGINT) รองรับ Railway Restart
  • รองรับหุ้นไทย, Crypto, Commodities ครบถ้วน
  • Log Injection Protection

Environment Variables (บน Railway → Variables):
-----------------------------------------------
TELEGRAM_TOKEN        : required (อย่า hardcode!)
...
PORT                  : default "8080" (Railway จะ inject อัตโนมัติ)
WEBHOOK_URL           : บังคับถ้าใช้ webhook เช่น https://yourbot.up.railway.app
WEBHOOK_SECRET        : optional (เพิ่มความปลอดภัย webhook)
"""
...
if not TELEGRAM_TOKEN:
    raise ValueError("TELEGRAM_TOKEN environment variable is required. Set it in Railway Variables.")
...
PORT = int(os.environ.get("PORT", "8080"))                     # Railway assigns PORT dynamically
WEBHOOK_URL = os.environ.get("WEBHOOK_URL", "")                  # e.g. https://yourbot.up.railway.app
WEBHOOK_SECRET = os.environ.get("WEBHOOK_SECRET", "")            # Optional webhook secret token

# =========================================================
# 🛡️ DEDUPLICATION & RATE LIMITING
# =========================================================
class UpdateDeduplicator:
    """กรอง update_id ที่ประมวลผลแล้ว เพื่อป้องกันการส่งซ้ำจาก Telegram Retry"""
    def __init__(self, max_size: int = 2000, ttl_seconds: int = 300):
        self._cache: Deque[int] = deque(maxlen=max_size)
    ...
    def is_duplicate(self, update_id: Optional[int]) -> bool:
        ...
        with self._lock:
            now = time.time()
            if len(self._cache) > self._cache.maxlen * 0.9:
                self._timestamps = {
                    k: v for k, v in self._timestamps.items()
                    if now - v < self._ttl and k in self._cache
                }
            if update_id in self._cache:
                logger.info("Deduplicated update_id=%s", update_id)
                return True
            self._cache.append(update_id)
            self._timestamps[update_id] = now
            return False


class UserRateLimiter:
    """จำกัดความถี่การขอวิเคราะห์ต่อ user ป้องกัน double-click / rapid-fire"""
...
class NewsCache:
    """แคชผลลัพธ์ format_news_section() ต่อ symbol ลดการเรียก OpenAI API ซ้ำ"""
...
class AISummaryManager:
    """
    จัดการ Cache + Lock สำหรับ AI Summary ต่อ Symbol
        ...
        with self._main_lock:
            if symbol in self._cache:
                ...
                if time.time() - ts < self._ttl:
                    logger.info("AI cache HIT for %s", symbol)
                    return result
                del self._cache[symbol]
            return None
...
# Singleton instances
update_dedup = UpdateDeduplicator(max_size=2000, ttl_seconds=300)
user_rate_limit = UserRateLimiter(cooldown_seconds=8.0)
news_cache = NewsCache(ttl_seconds=300)
ai_summary_mgr = AISummaryManager(ttl_seconds=300)
...
# =========================================================
# INPUT SANITIZATION & SYMBOL FORMATTING (NEW)
# =========================================================
THAI_STOCKS: Set[str] = {
...
def sanitize_input(text: str) -> Optional[str]:
    ...
    คืนค่า None ถ้าไม่ผ่าน validation
    """
    if not text or not isinstance(text, str):
        return None
    # Normalize unicode (ป้องกัน homograph attack)
    text = unicodedata.normalize('NFKC', text)
    # Strip whitespace และแปลง uppercase
...
def format_symbol(symbol: str) -> str:
    """
    แปลงชื่อหุ้นที่ผู้ใช้พิมพ์ให้เป็นรูปแบบที่ yfinance เข้าใจ
    """
    symbol = symbol.upper().strip()

    # กรณีผู้ใช้พิมพ์มาพร้อม suffix อยู่แล้ว
    ...
    # หุ้นสหรัฐและอื่นๆ (ส่งตรงๆ ให้ yfinance จัดการ)
    return symbol


def is_valid_stock_input(text: str) -> bool:
    """
    Validate ว่าเป็นชื่อหุ้นที่เป็นไปได้
    รองรับ: US stocks, Thai stocks, Crypto, Commodities, Indices
    """
    if not text:
        return False
    ...
    # Pattern ที่รองรับหุ้นสหรัฐ, ไทย, crypto, commodities, indices
    # เช่น AAPL, BRK.B, GC=F, BTC-USD, PTT.BK, ^GSPC, TCEHY
    pattern = r'^[A-Z\^][A-Z0-9\.\-\_\=]{0,19}$'
    if not re.match(pattern, clean):
        return False

    # ป้องกัน path traversal หรือ command injection
    dangerous = ['../', '..\\', '`', '$(', '${']
...
# =========================================================
# UNIFIED DATA FETCHER (with LRU Memory Cache for Railway)
# =========================================================
class DataFetcher:
    def __init__(self):
        self._df_cache: Dict[str, Tuple[float, pd.DataFrame]] = {}
        self._cache_ttl = 300  # 5 นาที
        self._max_cache_size = 50  # จำกัดจำนวนหุ้นใน cache
        ...
        with self._cache_lock:
            ...
            if symbol in self._df_cache:
                ...
                if now - ts < self._cache_ttl:
                    logger.info("Data cache HIT for %s", symbol)
                    return df
    ...
    def _set_cache(self, symbol: str, df: pd.DataFrame):
        with self._cache_lock:
            # Eviction ถ้า cache เต็ม (LRU-style)
    ...
    def fetch_ohlcv(self, symbol: str, period: str = "2y", interval: str = None):
        ...
        try:
            ...
            if len(df) < 100:
                ...
                return None
            logger.info("Successfully fetched %s rows for %s", len(df), symbol)
...
class ChartGenerator:
    ...
    def generate_trading_chart(self, df: pd.DataFrame, symbol: str,
        ...
        ax1.set_title(f'Investockify Bot v{VERSION} — {symbol} | Pivot S/R + POC/VAH/VAL + BB\n'
        ...
        if 'Volume' in df_plot.columns:
            ...
            # ตั้งค่าแกน Y ให้อ่านง่าย (M = ล้าน, B = พันล้าน)
...
# =========================================================
# UX HELPER — สร้างข้อความบริบทตามสถานการณ์
# =========================================================

def _get_entry_context(distance: float, trend: str, weekly_trend: str) -> str:
    """
    สร้างข้อความบริบทเฉพาะสถานการณ์ เชื่อมโยง Entry Timing กับ Trend
    ไม่เปลี่ยนตรรกะการคำนวณ แต่เพิ่มคำอธิบายให้ผู้ใช้เข้าใจว่า
    ทำไมถึงเป็น "รอซื้อ" ทั้งที่แนวโน้มเป็นขาขึ้น (หรือกลับกัน)

    Parameters
    ----------
    distance : float
        ระยะห่างราคาปัจจุบันจากจุดเข้า (เปอร์เซ็นต์)
    trend : str
        แนวโน้มรายวัน ("BULLISH" | "BEARISH" | "SIDEWAYS")
    weekly_trend : str
        แนวโน้มรายสัปดาห์ ("BULLISH" | "BEARISH" | "SIDEWAYS" | "UNKNOWN")

    Returns
    -------
    str
        ข้อความคำแนะนำ 1 ย่อหน้า ภาษาไทย
    """
    # --- กรณี 1: ราคาสูงกว่าโซนซื้อมาก (> +5%) ---
    if distance > 5:
        if trend == "BULLISH" and weekly_trend == "BULLISH":
            return (
                "ราคาห่างจากโซนซื้อมาก แม้แนวโน้มจะขึ้นแรง "
                "แต่ควรรอจังหวะ Pullback เข้าใกล้จุดเข้าจึงจะปลอดภัย"
            )
        elif trend == "BEARISH" or weekly_trend == "BEARISH":
            return (
                "ราคาสูงเกินไปทั้งจุดเข้าและแนวโน้ม ไม่แนะนำให้ซื้อตอนนี้ "
                "ควรรอจนกว่าจะเข้าโซนซื้อและแนวโน้มกลับตัว"
            )
        else:
            return "ราคาห่างจากโซนซื้อ ควรรอจังหวะที่ดีกว่า"

    # --- กรณี 2: ราคาอยู่ในโซนซื้อ (-2% ถึง +5%) ---
    elif distance > -2:
        if trend == "BULLISH" and weekly_trend == "BULLISH":
            return (
                "จังหวะดีมาก ราคาอยู่ในโซนซื้อและแนวโน้มขึ้นแกร่ง "
                "เหมาะกับการตั้งคำสั่งซื้อ"
            )
        elif trend == "BEARISH" and weekly_trend == "BEARISH":
            return (
                "ราคาอยู่ในโซนซื้อ แต่ตลาดเป็นขาลง ควรระมัดระวัง "
                "หรือรอแนวโน้มกลับตัว"
            )
        else:
            return "ราคาอยู่ในโซนซื้อ ให้พิจารณาตามแนวโน้มประกอบ"

    # --- กรณี 3: ราคาต่ำกว่าเป้า (<= -2%) ---
    else:
        if trend == "BULLISH" and weekly_trend in ("BULLISH", "SIDEWAYS"):
            return (
                "ราคาต่ำกว่าโซนซื้อ อาจเป็นโอกาสซื้อสะสม (Dip Buying) "
                "หากแนวโน้มหลักยังเป็นขาขึ้น"
            )
        elif trend == "BEARISH" or weekly_trend == "BEARISH":
            return (
                "ราคาต่ำกว่าเป้าในตลาดขาลง อาจเป็นสัญญาณอันตราย "
                "ควรรอดูการฟื้นตัวของแนวโน้มก่อน"
            )
        else:
...
# =========================================================
# REFACTORED get_trend_badge() → v2
# =========================================================

def get_trend_badge_v2(trend: str, weekly_trend: str) -> str:
    """
    แสดงสภาวะตลาด (Market Structure) แทนการสั่งการซื้อ/ขายโดยตรง

    เปลี่ยนจากคำสั่ง "ซื้อได้ / งดซื้อ / รอ Pullback"
    เป็นคำอธิบายสภาวะตลาด เพื่อให้ผู้ใช้เข้าใจว่า
    นี่คือ "ภาพใหญ่" ไม่ใช่ "คำสั่งซื้อขาย" ที่ต้องทำตามทันที
...
# =========================================================
# REFACTORED get_smart_summary() → v2
# =========================================================

def get_smart_summary_v2(setup, current_price: float, trend: str, weekly_trend: str) -> str:
    """
    รวม Entry Timing + Trend Context เป็นข้อความเดียวที่มีบริบทชัดเจน

    แก้ปัญหาเดิมที่ "⏳ รอซื้อ" กับ "🟢🟢 STRONG UPTREND | ซื้อได้"
    แสดงพร้อมกันโดยไม่มีคำอธิบายว่าทำไมถึงขัดแย้ง

    ตรรกะการคำนวณ distance และ R:R ยังคงเหมือนเดิมทุกประการ
    เปลี่ยนเฉพาะการจัดรูปแบบข้อความให้ผู้ใช้เข้าใจว่า
    "จังหวะเข้า" กับ "แนวโน้มตลาด" เป็นสองมิติที่แยกจากกัน

    Parameters
    ----------
    setup : TradeSetup
        อ็อบเจกต์จาก investockify_bot11.py (ต้องมี .entry, .rr1)
    current_price : float
        ราคาปัจจุบัน
    trend : str
        แนวโน้มรายวัน
    ...
    -------
    str
        ข้อความสรุปแบบชั้นข้อมูลชัดเจน พร้อมบริบท
    """
    if setup is None:
        return "⚠️ ไม่สามารถวิเคราะห์ได้"

    # --- ตรรกะการคำนวณเดิมไม่เปลี่ยน ---
    ...
    # --- UX Fix: แสดงผลแบบชั้นข้อมูลชัดเจน ---
    lines = [
        #"━━━━━━━━━━━━━━━━━━━━",
...
# =========================================================
# Layer 1: Regex Pattern Matching (fast, deterministic, free)
BULLISH_PATTERNS = [
...
def _score_news_item(title: str, use_ai_fallback: bool = True) -> tuple:
    ...
    if use_ai_fallback and OPENAI_API_KEY and abs(bull_count - bear_count) <= 1:
        ...
        if cached is not None:
            # cached เป็น string จาก AI เก่า ต้องแปลงกลับเป็น tuple
            # เก็บในรูปแบบที่ parse ได้
            pass
        ...
        try:
            ...
            pass  # Fallback ไปใช้ keyword ถ้า AI ล้มเหลว
...
async def send_real_stock_analysis(message, stock: str):
    ...
    try:
        ...
        # Log ที่ปลอดภัย (ไม่ใช้ f-string กับ input โดยตรง)
...
# =========================================================
# MESSAGE HANDLER (แก้ไขป้องกันการส่งซ้ำ + Rate Limit)
# =========================================================
...
# =========================================================
def main():
    logger.info("Starting Investockify Bot v%s — Railway Production Edition", VERSION)
